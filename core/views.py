...
from .forms import Verificacion2FAForm, CustomLoginForm
...
class CustomLoginView(LoginView):
    ...
    def form_valid(self, form):
        ...
        messages.info(
            self.request, f"Se envió el código de verificación a los administradores."
        )
...
class Verificar2FAView(FormView):
    ...
    def form_invalid(self, form):
        ...
        if any('EXPIRED' in error for error in form.non_field_errors()):
            self._limpiar_sesion_temp()
            messages.error(self.request, 'El código expiró. Vuelva a iniciar sesión.')
            return redirect('login')
...
class Inicio(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        messages.error(request, "Ocurrió un error grave al procesar la solicitud.")
